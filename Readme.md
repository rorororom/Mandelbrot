...
### Процесс создания изображения множества Мандельброта
Процесс создания изображения множества Мандельброта включает в себя итерацию для каждой точки на комплексной плоскости. Для каждой точки $(x,y)$ на плоскости вычисляется значение $c=x+yi$, а затем для этого значения $c$ итерационно вычисляется последовательность $z_{n+1}=z_{n}^2+c$ до тех пор, пока не будет достигнуто максимальное количество итераций или модуль $z_{n}$ не превысит определенного порогового значения. Количество итераций, которое требуется для превышения порога, определяет цвет или оттенок пикселя, представляющего эту точку на изображении.
